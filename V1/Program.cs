...
namespace MyDriverBoosterInstaller
{
    class Program
    {
        ...
        static void Main(string[] args)
        {
            ...
            string destinationDirectory = @"C:\IOBit\DriverBooster";
            ...
            string resourceName = "DriverInstaller.Resources.DriverBooster.zip"; // À adapter si besoin
            ...
            string mainExePath = Path.Combine(destinationDirectory, "DriverBoosterPortable.exe");
            if (File.Exists(mainExePath))
            {
                // Demande de confirmation via une boîte de dialogue
                DialogResult result = MessageBox.Show(
                    "Voulez-vous lancer Driver Booster ?", 
                    "Confirmation", 
                ...
                {
                    Logger.LogInfo("Lancement de Driver Booster ...");
                    try
                    {
                    ...
                    CreateShortcut("IObit Driver Booster Pro", mainExePath);
                    Logger.LogInfo("Raccourci créé sur le Bureau.");
                }
                else
                {
                    Logger.LogInfo("Lancement de Driver Booster annulé par l'utilisateur.");
                }
            }
            else
            {
                Logger.LogError("Le fichier DriverBoosterPortable.exe n'a pas été trouvé dans les fichiers extraits.");
            }
        }
